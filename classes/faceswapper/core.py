...
from faceswapper.utilities import has_image_extension, is_image, is_video, detect_fps, create_video, extract_frames, get_temp_frame_paths, restore_audio, create_temp, move_temp, clean_temp, normalize_output_path
...
def parse_args() -> None:
    ...
    program.add_argument('-v', '--version', action='version', version=f'{faceswapper.metadata.name} {faceswapper.metadata.version}')
    ...
    faceswapper.globals.frame_processors = args.frame_processor
    faceswapper.globals.keep_fps = args.keep_fps
    faceswapper.globals.keep_frames = args.keep_frames
    faceswapper.globals.skip_audio = args.skip_audio
    faceswapper.globals.many_faces = args.many_faces
    ...
    faceswapper.globals.similar_face_distance = args.similar_face_distance
    ...
    faceswapper.globals.execution_providers = decode_execution_providers(args.execution_provider)
    faceswapper.globals.execution_threads = args.execution_threads
...
def update_status(message: str, scope: str = 'faceswapper.CORE') -> None:
    ...
    if not faceswapper.globals.headless:
...
def start() -> None:
    ...
    if has_image_extension(faceswapper.globals.target_path):
        ...
        for frame_processor in get_frame_processors_modules(faceswapper.globals.frame_processors):
            ...
            frame_processor.process_image(faceswapper.globals.source_path, faceswapper.globals.output_path, faceswapper.globals.output_path)
    ...
    if temp_frame_paths:
        for frame_processor in get_frame_processors_modules(faceswapper.globals.frame_processors):
            update_status('Progressing...', frame_processor.NAME)
            frame_processor.process_video(faceswapper.globals.source_path, temp_frame_paths)
            frame_processor.post_process()
    else:
    ...
    if faceswapper.globals.keep_fps:
        ...
        create_video(faceswapper.globals.target_path, fps)
    else:
        update_status('Creating video with 30 FPS...')
        create_video(faceswapper.globals.target_path)
    # handle audio
