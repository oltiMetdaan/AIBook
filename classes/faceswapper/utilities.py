...
def run_ffmpeg(args: List[str]) -> bool:
    commands = ['ffmpeg', '-hide_banner', '-loglevel', faceswapper.globals.log_level]
...
def extract_frames(target_path: str, fps: float = 30) -> bool:
    ...
    temp_frame_quality = faceswapper.globals.temp_frame_quality * 31 // 100
    return run_ffmpeg(['-hwaccel', 'auto', '-i', target_path, '-q:v', str(temp_frame_quality), '-pix_fmt', 'rgb24', '-vf', 'fps=' + str(fps), os.path.join(temp_directory_path, '%04d.' + faceswapper.globals.temp_frame_format)])


def create_video(target_path: str, fps: float = 30) -> bool:
    temp_output_path = get_temp_output_path(target_path)
    temp_directory_path = get_temp_directory_path(target_path)
    output_video_quality = (faceswapper.globals.output_video_quality + 1) * 51 // 100
...
def clean_temp(target_path: str) -> None:
    ...
    if not faceswapper.globals.keep_frames and os.path.isdir(temp_directory_path):
